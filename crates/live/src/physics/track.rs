/// Track data loading for live racing physics.
///
...
use crate::physics::wasm_engine::Obstacle;
...
/// Loaded track data ready for WASM physics
#[derive(Debug, Clone)]
...
impl TrackStore {
    ...
    ///
    /// The directory should contain track JSON files.
    ...
    /// Convert TrackJson to TrackData (WASM format)
    fn convert_track(json: &TrackJson) -> Result<TrackData> {
        ...
        for obs in &json.obstacles {
            let obstacle_type = match obs.obstacle_type.as_str() {
                "box" => crate::physics::wasm_engine::ObstacleType::Box,
                "circle" => crate::physics::wasm_engine::ObstacleType::Circle,
                _ => {
                    tracing::warn!(
                        track = %json.id,
                    ...
                    crate::physics::wasm_engine::ObstacleType::Box
                }
            };
        ...
        // WASM physics engine. They're loaded here for future use but not
