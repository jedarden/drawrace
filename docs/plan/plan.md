...
- **Stuck-DNF (no-progress detection)**: if the player's vehicle spins its wheels through **10 full rotations without advancing the chassis by more than 0.5 m** along +x, the race ends as DNF and returns to the Result screen with a "stuck" message. The rotation counter is accumulated per-tick as `Σ |ω_front| + |ω_rear|) × dt / (2π × 2)` (half-sum across both AWD wheels, converting radians to whole-car-rotations); the progress baseline is `chassis.position.x` at the time the counter last reset. **Swapping the wheel resets both the rotation counter and the progress baseline**, giving the player a rescue attempt — redraw into a better shape and you get a fresh 10 rotations to recover. Why this is needed: a flipped-upside-down vehicle, a wheel wedged against an obstacle, or a pathologically angular shape that spins in place all leave the 2×-track-best timeout too permissive (the player sits for 90s watching nothing happen). 10 rotations at motor max speed (8 rad/s ≈ 1.27 rev/s) is ~4 s of motor runtime — long enough to not false-positive on snow/mud slowdowns but short enough to get the player back to the Draw or Retry screen quickly. Deterministic because it's tick-indexed; validator re-sim runs the same detection and **rejects any submitted finish whose re-sim would have triggered stuck-DNF** (a forgery signal).
...
| Car flipped upside down or wedged (wheels spin, chassis doesn't advance) | 10 full wheel rotations with `chassis.position.x` delta < 0.5 m | Stuck-DNF — same "DNF runs are never submitted" path as timeout-DNF (see §Gameplay 1). Rescue: swapping the wheel resets the counter and baseline, so a timely redraw can save the run. |
...
- Server rejects any submission whose re-simulation (Layer 3) would have triggered **stuck-DNF** (10-rotation no-progress detection per §Gameplay 1). A run that claims a real finish time but whose `wheels[]` sequence produces a stuck state at any point under re-sim is by definition a forgery — the client would have DNFd instead of submitting.
