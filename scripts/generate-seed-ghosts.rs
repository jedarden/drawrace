...
fn main() -> Result<()> {
    ...
    fs::create_dir_all(&seeds_dir)
        .with_context(|| format!("Failed to create seeds directory: {:?}", seeds_dir))?;
    ...
    for (i, seed) in SEEDS.iter().enumerate() {
        ...
        fs::write(&filepath, blob)
            .with_context(|| format!("Failed to write seed file: {:?}", filepath))?;

        println!("  Wrote {}: {} ({})", filename, seed.name, seed.time_ms);
    }

    println!("\nGenerated {} seed ghost blob files in {:?}", SEEDS.len(), seeds_dir);
    println!("\nBucket distribution:");
